...
.main{animation:pageIn .6s cubic-bezier(.16,1,.3,1) backwards}
@keyframes pageIn{from{opacity:0;transform:translateY(16px)}to{opacity:1}}
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>NexStock — Smart Inventory System</title>
<meta name="description" content="AI-powered barcode scanning, real-time stock tracking, and expiry management. Built for speed.">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
<style>
*{margin:0;padding:0;box-sizing:border-box}
:root{
  --bg:#050505;--fg:#f0f0f0;--muted:#525252;--accent:#22c55e;--accent2:#4ade80;
  --card:#0a0a0a;--border:#1a1a1a;
}
html{scroll-behavior:smooth;overflow-x:hidden}
body{background:var(--bg);color:var(--fg);font-family:'Inter',sans-serif;overflow-x:hidden;-webkit-font-smoothing:antialiased}
::selection{background:var(--accent);color:#000}

/* ── Loading Screen ── */
.ld-screen{position:fixed;inset:0;z-index:9999;background:var(--accent);display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity .6s,visibility .6s}
.ld-screen.hide{opacity:0;visibility:hidden;pointer-events:none}
.ld-logo{font-family:'JetBrains Mono',monospace;font-size:clamp(1.4rem,3vw,2.2rem);font-weight:700;color:#000;letter-spacing:6px}
.ld-bar{width:120px;height:2px;background:#00000030;margin-top:24px;position:relative;overflow:hidden}
.ld-bar::after{content:'';position:absolute;left:0;top:0;height:100%;width:0;background:#000;animation:ldFill 1.8s ease-in-out forwards}
.ld-sub{font-family:'JetBrains Mono',monospace;font-size:.65rem;color:#00000080;letter-spacing:4px;margin-top:12px;text-transform:uppercase}
@keyframes ldFill{0%{width:0}100%{width:100%}}

/* ── Nav ── */
.ln-nav{position:fixed;top:0;left:0;right:0;z-index:100;padding:20px 40px;display:flex;align-items:center;justify-content:space-between;transition:background .3s}
.ln-nav.scrolled{background:rgba(5,5,5,.92);backdrop-filter:blur(12px);border-bottom:1px solid #ffffff08}
.ln-brand{font-family:'JetBrains Mono',monospace;font-size:1.1rem;font-weight:700;letter-spacing:4px;color:var(--fg);text-decoration:none}
.ln-brand span{color:var(--accent)}
.ln-right{display:flex;align-items:center;gap:20px}
.ln-menu-btn{width:36px;height:36px;border:1px solid #ffffff20;background:transparent;cursor:pointer;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:4px;transition:border-color .3s}
.ln-menu-btn:hover{border-color:var(--accent)}
.ln-menu-btn span{width:16px;height:1.5px;background:var(--fg);transition:transform .3s}
.ln-cta{font-family:'JetBrains Mono',monospace;font-size:.72rem;font-weight:700;letter-spacing:3px;padding:10px 24px;background:var(--fg);color:var(--bg);text-decoration:none;display:flex;align-items:center;gap:8px;transition:background .3s,color .3s}
.ln-cta:hover{background:var(--accent);color:#000}
.ln-cta svg{width:14px;height:14px}

/* ── Menu Overlay ── */
.ln-overlay{position:fixed;inset:0;z-index:99;background:rgba(5,5,5,.96);backdrop-filter:blur(20px);display:flex;align-items:center;justify-content:center;opacity:0;visibility:hidden;transition:opacity .4s,visibility .4s}
.ln-overlay.open{opacity:1;visibility:visible}
.ln-overlay-links{display:flex;flex-direction:column;gap:24px;text-align:center}
.ln-overlay-links a{font-family:'Inter',sans-serif;font-size:clamp(2rem,5vw,4rem);font-weight:800;color:var(--fg);text-decoration:none;letter-spacing:-1px;text-transform:uppercase;opacity:.6;transition:opacity .3s,color .3s}
.ln-overlay-links a:hover{opacity:1;color:var(--accent)}
.ln-close{position:absolute;top:24px;right:40px;width:40px;height:40px;border:1px solid #ffffff20;background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1.2rem;color:var(--fg)}

/* ── Hero ── */
.ln-hero{min-height:100vh;display:flex;flex-direction:column;justify-content:center;padding:140px 40px 80px;position:relative;overflow:hidden}
.ln-hero-tag{font-family:'JetBrains Mono',monospace;font-size:.65rem;letter-spacing:4px;color:var(--accent);text-transform:uppercase;margin-bottom:32px;display:flex;align-items:center;gap:12px}
.ln-hero-tag::before{content:'';width:24px;height:2px;background:var(--accent)}
.ln-hero-title{font-size:clamp(3rem,10vw,9rem);font-weight:900;line-height:.92;letter-spacing:-3px;text-transform:uppercase;max-width:1200px}
.ln-hero-title .outline{color:transparent;-webkit-text-stroke:1.5px rgba(240,240,240,.25)}
.ln-hero-title .green{color:var(--accent)}
.ln-hero-sub{font-size:clamp(.85rem,1.2vw,1rem);color:var(--muted);max-width:440px;margin-top:40px;line-height:1.7;font-weight:400}
.ln-hero-sub strong{color:var(--fg);font-weight:600}
.ln-hero-btns{display:flex;gap:12px;margin-top:40px;flex-wrap:wrap}
.ln-btn-primary{font-family:'JetBrains Mono',monospace;font-size:.72rem;font-weight:700;letter-spacing:3px;padding:14px 32px;background:var(--fg);color:var(--bg);text-decoration:none;display:inline-flex;align-items:center;gap:10px;transition:background .3s,color .3s;text-transform:uppercase}
.ln-btn-primary:hover{background:var(--accent);color:#000}
.ln-btn-outline{font-family:'JetBrains Mono',monospace;font-size:.72rem;font-weight:700;letter-spacing:3px;padding:14px 32px;background:transparent;color:var(--fg);text-decoration:none;display:inline-flex;align-items:center;gap:10px;border:1px solid #ffffff20;transition:border-color .3s,color .3s;text-transform:uppercase}
.ln-btn-outline:hover{border-color:var(--accent);color:var(--accent)}
.ln-scroll-hint{position:absolute;bottom:40px;left:40px;font-family:'JetBrains Mono',monospace;font-size:.55rem;letter-spacing:4px;color:var(--muted);text-transform:uppercase;writing-mode:vertical-lr;display:flex;align-items:center;gap:12px}
.ln-scroll-hint::after{content:'';width:1.5px;height:40px;background:var(--muted);animation:scrollPulse 2s ease-in-out infinite}
@keyframes scrollPulse{0%,100%{opacity:.3;transform:scaleY(.6)}50%{opacity:1;transform:scaleY(1)}}

/* ── SVG Background Pattern ── */
.ln-topo{position:absolute;inset:0;pointer-events:none;opacity:.04}
.ln-topo svg{width:100%;height:100%}

/* ── Marquee ── */
.ln-marquee{padding:20px 0;border-top:1px solid var(--border);border-bottom:1px solid var(--border);overflow:hidden;white-space:nowrap}
.ln-marquee-track{display:inline-flex;animation:marquee 25s linear infinite}
.ln-marquee-track span{font-family:'JetBrains Mono',monospace;font-size:.7rem;letter-spacing:5px;text-transform:uppercase;color:var(--muted);padding:0 32px;display:inline-flex;align-items:center;gap:20px}
.ln-marquee-track span::after{content:'';width:4px;height:4px;background:var(--accent);border-radius:50%}
@keyframes marquee{0%{transform:translateX(0)}100%{transform:translateX(-50%)}}

/* ── Statement Section (Lando-style giant text) ── */
.ln-statement{padding:160px 40px;text-align:center}
.ln-statement-text{font-size:clamp(2rem,5.5vw,5rem);font-weight:800;line-height:1.15;letter-spacing:-1px;max-width:1100px;margin:0 auto;text-transform:uppercase}
.ln-statement-text .accent{color:var(--accent);font-style:italic}
.ln-statement-text .dim{color:#ffffff30}

/* ── Features ── */
.ln-features{padding:120px 40px}
.ln-section-tag{font-family:'JetBrains Mono',monospace;font-size:.6rem;letter-spacing:5px;color:var(--accent);text-transform:uppercase;margin-bottom:16px;display:flex;align-items:center;gap:12px}
.ln-section-tag::before{content:'';width:32px;height:1px;background:var(--accent)}
.ln-section-title{font-size:clamp(2rem,4vw,3.5rem);font-weight:800;letter-spacing:-1px;text-transform:uppercase;margin-bottom:64px}
.ln-feat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:1px;background:var(--border)}
.ln-feat-card{background:var(--bg);padding:48px 36px;transition:background .3s}
.ln-feat-card:hover{background:#0a0a0a}
.ln-feat-num{font-family:'JetBrains Mono',monospace;font-size:.6rem;color:var(--muted);letter-spacing:3px;margin-bottom:24px}
.ln-feat-icon{width:40px;height:40px;margin-bottom:20px}
.ln-feat-icon svg{width:100%;height:100%;stroke:var(--accent);fill:none;stroke-width:1.5}
.ln-feat-name{font-family:'JetBrains Mono',monospace;font-size:.85rem;font-weight:700;letter-spacing:3px;text-transform:uppercase;margin-bottom:12px}
.ln-feat-desc{font-size:.85rem;color:var(--muted);line-height:1.7}

/* ── How it works ── */
.ln-steps{padding:120px 40px;border-top:1px solid var(--border)}
.ln-steps-grid{display:flex;gap:0;margin-top:64px;flex-wrap:wrap}
.ln-step{flex:1;min-width:260px;padding:48px 36px;border-right:1px solid var(--border)}
.ln-step:last-child{border-right:none}
.ln-step-num{font-family:'JetBrains Mono',monospace;font-size:3rem;font-weight:700;color:var(--accent);opacity:.3;margin-bottom:20px}
.ln-step-title{font-family:'JetBrains Mono',monospace;font-size:.8rem;font-weight:700;letter-spacing:3px;text-transform:uppercase;margin-bottom:12px}
.ln-step-desc{font-size:.85rem;color:var(--muted);line-height:1.7}

/* ── FAQ ── */
.ln-faq{padding:120px 40px;border-top:1px solid var(--border)}
.ln-faq-list{max-width:800px;margin-top:48px}
.ln-faq-item{border-bottom:1px solid var(--border)}
.ln-faq-q{font-family:'JetBrains Mono',monospace;font-size:.8rem;font-weight:600;letter-spacing:1px;padding:24px 0;cursor:pointer;display:flex;justify-content:space-between;align-items:center;color:var(--fg);transition:color .3s}
.ln-faq-q:hover{color:var(--accent)}
.ln-faq-q svg{width:16px;height:16px;stroke:currentColor;transition:transform .3s}
.ln-faq-q.open svg{transform:rotate(45deg)}
.ln-faq-a{font-size:.85rem;color:var(--muted);line-height:1.7;padding:0 0 24px;display:none}
.ln-faq-a.open{display:block}

/* ── CTA ── */
.ln-cta-section{padding:160px 40px;text-align:center;position:relative;overflow:hidden}
.ln-cta-title{font-size:clamp(2.5rem,6vw,6rem);font-weight:900;letter-spacing:-2px;text-transform:uppercase;margin-bottom:24px}
.ln-cta-title .green{color:var(--accent)}
.ln-cta-sub{font-size:1rem;color:var(--muted);margin-bottom:40px}

/* ── Footer ── */
.ln-footer{padding:40px;border-top:1px solid var(--border);display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:16px}
.ln-footer-left{font-family:'JetBrains Mono',monospace;font-size:.6rem;color:var(--muted);letter-spacing:3px;text-transform:uppercase}
.ln-footer-right{display:flex;gap:24px}
.ln-footer-right a{font-family:'JetBrains Mono',monospace;font-size:.6rem;color:var(--muted);letter-spacing:2px;text-decoration:none;text-transform:uppercase;transition:color .3s}
.ln-footer-right a:hover{color:var(--accent)}
...
@media(max-width:768px){
  .ln-nav{padding:16px 20px}
  .ln-hero{padding:120px 20px 60px}
  .ln-hero-title{font-size:clamp(2.5rem,12vw,4.5rem);letter-spacing:-1px}
  .ln-statement{padding:80px 20px}
  .ln-features,.ln-steps,.ln-faq,.ln-cta-section{padding:80px 20px}
  ...
  .ln-footer{padding:24px 20px;flex-direction:column;text-align:center}
  .ln-scroll-hint{display:none}
  .ln-cta{padding:10px 16px;font-size:.65rem}
}
</style>
</head>
<body>
...
<!-- Loading Screen -->
<div class="ld-screen" id="ldScreen">
  <div class="ld-logo">NEXSTOCK</div>
  <div class="ld-bar"></div>
  <div class="ld-sub">initializing</div>
</div>

<!-- Nav -->
<nav class="ln-nav" id="lnNav">
  <a href="/" class="ln-brand">NEX<span>STOCK</span></a>
  <div class="ln-right">
    <a href="/giris" class="ln-cta">OPEN APP <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
    <button class="ln-menu-btn" onclick="document.getElementById('lnOverlay').classList.add('open')">
...
<div class="ln-overlay" id="lnOverlay">
  <button class="ln-close" onclick="this.parentElement.classList.remove('open')">&times;</button>
  <div class="ln-overlay-links">
    <a href="#features" onclick="this.closest('.ln-overlay').classList.remove('open')">Features</a>
    <a href="#about" onclick="this.closest('.ln-overlay').classList.remove('open')">About</a>
    <a href="#faq" onclick="this.closest('.ln-overlay').classList.remove('open')">FAQ</a>
    <a href="/giris" style="color:var(--accent);opacity:1">Sign In</a>
  </div>
</div>

<!-- Hero -->
<section class="ln-hero">
  <div class="ln-topo"><svg viewBox="0 0 1440 900" xmlns="http://www.w3.org/2000/svg"><path d="M0 450Q360 350 720 450T1440 450" fill="none" stroke="#fff" stroke-width=".5"/><path d="M0 380Q360 280 720 380T1440 380" fill="none" stroke="#fff" stroke-width=".4"/><path d="M0 520Q360 420 720 520T1440 520" fill="none" stroke="#fff" stroke-width=".3"/><path d="M0 300Q400 200 800 300T1440 250" fill="none" stroke="#fff" stroke-width=".3"/><path d="M0 600Q400 500 800 600T1440 580" fill="none" stroke="#fff" stroke-width=".3"/><circle cx="200" cy="350" r="120" fill="none" stroke="#fff" stroke-width=".3"/><circle cx="1100" cy="500" r="180" fill="none" stroke="#fff" stroke-width=".2"/><circle cx="700" cy="200" r="80" fill="none" stroke="#fff" stroke-width=".25"/></svg></div>
  <div class="ln-hero-tag">DFC Turkey Competition — 2026</div>
  <h1 class="ln-hero-title">
    NEXSTOCK<br>
    <span class="outline">THE FUTURE</span><br>
    <span class="green">OF INVENTORY.</span>
  </h1>
  <p class="ln-hero-sub">A <strong>smart barcode and inventory</strong> management system. Real-time scanning, AI-powered insights, expiry tracking. Built for speed.</p>
  <div class="ln-hero-btns">
    <a href="/kayit" class="ln-btn-primary">GET STARTED <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
    <a href="#features" class="ln-btn-outline">LEARN MORE</a>
  </div>
  <div class="ln-scroll-hint">Scroll</div>
</section>

<!-- Marquee -->
<div class="ln-marquee">
  <div class="ln-marquee-track">
    <span>SCAN</span><span>TRACK</span><span>MANAGE</span><span>DESIGN FOR CHANGE</span><span>NEXSTOCK</span><span>AI READER</span><span>REAL-TIME</span><span>FEFO</span>
    <span>SCAN</span><span>TRACK</span><span>MANAGE</span><span>DESIGN FOR CHANGE</span><span>NEXSTOCK</span><span>AI READER</span><span>REAL-TIME</span><span>FEFO</span>
  </div>
</div>

<!-- Statement -->
<section class="ln-statement">
  <div class="ln-statement-text reveal">
    <span class="accent">Scanning</span> barcodes,<br>
    <span class="dim">tracking</span> expiry,<br>
    managing stock <span class="accent">in real time.</span><br>
...
<section class="ln-features" id="features">
  <div class="ln-section-tag">// Features</div>
  <h2 class="ln-section-title">Why NexStock?</h2>
  <div class="ln-feat-grid">
    <div class="ln-feat-card reveal">
      <div class="ln-feat-num">01/06</div>
      <div class="ln-feat-icon"><svg viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2"/><line x1="6" y1="8" x2="6" y2="16"/><line x1="10" y1="8" x2="10" y2="16"/><line x1="14" y1="8" x2="14" y2="16"/><line x1="18" y1="8" x2="18" y2="16"/></svg></div>
      <div class="ln-feat-name">Camera Scan</div>
      <div class="ln-feat-desc">Real-time barcode reading via browser camera. EAN-13, EAN-8, UPC-A, Code 128 — all formats supported.</div>
    </div>
    <div class="ln-feat-card reveal">
      <div class="ln-feat-num">02/06</div>
      <div class="ln-feat-icon"><svg viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16z"/><polyline points="3.27 6.96 12 12.01 20.73 6.96"/><line x1="12" y1="22.08" x2="12" y2="12"/></svg></div>
      <div class="ln-feat-name">Batch System</div>
      <div class="ln-feat-desc">Track different-dated batches of the same product separately. FEFO algorithm prioritizes nearest expiry automatically.</div>
    </div>
    <div class="ln-feat-card reveal">
      <div class="ln-feat-num">03/06</div>
      <div class="ln-feat-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div>
      <div class="ln-feat-name">Expiry Tracking</div>
      <div class="ln-feat-desc">Color-coded alerts for approaching expiry dates. Never lose a product to waste again.</div>
    </div>
    <div class="ln-feat-card reveal">
      <div class="ln-feat-num">04/06</div>
      <div class="ln-feat-icon"><svg viewBox="0 0 24 24"><path d="M12 2a10 10 0 0110 10c0 5.52-4.48 10-10 10S2 17.52 2 12"/><path d="M12 2v4"/><path d="M2 12h4"/><circle cx="12" cy="12" r="3"/></svg></div>
      <div class="ln-feat-name">AI Reader</div>
      <div class="ln-feat-desc">AI-powered product data completion. Crowdsource missing nutrition info from Open Food Facts global database.</div>
    </div>
    <div class="ln-feat-card reveal">
      <div class="ln-feat-num">05/06</div>
      <div class="ln-feat-icon"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87"/><path d="M16 3.13a4 4 0 010 7.75"/></svg></div>
      <div class="ln-feat-name">Multi-Role</div>
      <div class="ln-feat-desc">Admin, user, and viewer roles. Role-based dashboards with personalized health warnings and allergen alerts.</div>
    </div>
    <div class="ln-feat-card reveal">
      <div class="ln-feat-num">06/06</div>
      <div class="ln-feat-icon"><svg viewBox="0 0 24 24"><rect x="1" y="4" width="22" height="16" rx="2" ry="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg></div>
      <div class="ln-feat-name">Smart Reports</div>
      <div class="ln-feat-desc">Visual dashboards, stock movement history, low-stock alerts, and exportable analytics.</div>
    </div>
  </div>
</section>

<!-- How it works -->
<section class="ln-steps" id="about">
  <div class="ln-section-tag">// How it works</div>
  <h2 class="ln-section-title">3 Simple Steps</h2>
  <div class="ln-steps-grid">
    <div class="ln-step reveal">
      <div class="ln-step-num">01</div>
      <div class="ln-step-title">Point the Camera</div>
      <div class="ln-step-desc">Open the scanner and point your device camera at any barcode. Instant recognition, zero delay.</div>
    </div>
    <div class="ln-step reveal">
      <div class="ln-step-num">02</div>
      <div class="ln-step-title">Product Info Loads</div>
      <div class="ln-step-desc">Product details, nutrition data, and stock status are fetched automatically from the database and Open Food Facts.</div>
    </div>
    <div class="ln-step reveal">
      <div class="ln-step-num">03</div>
      <div class="ln-step-title">Manage Your Stock</div>
      <div class="ln-step-desc">Add batches, set expiry dates, track quantities. The system handles FEFO ordering and alerts for you.</div>
    </div>
  </div>
</section>

<!-- FAQ -->
<section class="ln-faq" id="faq">
  <div class="ln-section-tag">// FAQ</div>
  <h2 class="ln-section-title">Questions</h2>
  <div class="ln-faq-list">
    <div class="ln-faq-item">
      <div class="ln-faq-q" onclick="toggleFaq(this)">What is NexStock? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div class="ln-faq-a">NexStock is a smart barcode-based inventory management system developed for the DFC Turkey 2026 competition. It combines real-time camera scanning, AI-powered data completion, and intelligent stock tracking with FEFO algorithm.</div>
    </div>
    <div class="ln-faq-item">
      <div class="ln-faq-q" onclick="toggleFaq(this)">Is it free to use? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div class="ln-faq-a">Yes. NexStock is completely free and open-source. Create an account and start scanning immediately.</div>
    </div>
    <div class="ln-faq-item">
      <div class="ln-faq-q" onclick="toggleFaq(this)">Which barcode formats are supported? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div class="ln-faq-a">EAN-13, EAN-8, UPC-A, and Code 128. The camera scanner recognizes all major retail barcode formats in real-time.</div>
    </div>
    <div class="ln-faq-item">
      <div class="ln-faq-q" onclick="toggleFaq(this)">What is the AI Reader? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div class="ln-faq-a">The AI Reader identifies products in the Open Food Facts database with missing or incomplete information. Users can contribute data to help improve the global food database — crowdsourced accuracy at scale.</div>
    </div>
    <div class="ln-faq-item">
      <div class="ln-faq-q" onclick="toggleFaq(this)">What is FEFO? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div class="ln-faq-a">First Expiry, First Out. The system automatically prioritizes products with the nearest expiry date, reducing food waste and ensuring stock rotation without manual effort.</div>
    </div>
    <div class="ln-faq-item">
      <div class="ln-faq-q" onclick="toggleFaq(this)">Do I need special hardware? <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
      <div class="ln-faq-a">No. NexStock runs entirely in the browser. Any device with a camera and internet connection works — phone, tablet, or laptop.</div>
    </div>
  </div>
</section>
...
<section class="ln-cta-section">
  <div class="ln-topo"><svg viewBox="0 0 1440 600" xmlns="http://www.w3.org/2000/svg"><path d="M0 300Q360 200 720 300T1440 300" fill="none" stroke="#fff" stroke-width=".5"/><path d="M0 250Q360 150 720 250T1440 250" fill="none" stroke="#fff" stroke-width=".3"/><path d="M0 350Q360 250 720 350T1440 350" fill="none" stroke="#fff" stroke-width=".3"/></svg></div>
  <h2 class="ln-cta-title reveal">START<br><span class="green">SCANNING.</span></h2>
  <p class="ln-cta-sub reveal">Free. Fast. No setup required.</p>
  <a href="/kayit" class="ln-btn-primary reveal">CREATE ACCOUNT <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>
</section>

<!-- Footer -->
<footer class="ln-footer">
  <div class="ln-footer-left">DFC Turkiye 2026 — Hortor</div>
  <div class="ln-footer-right">
    <a href="#features">Features</a>
    <a href="#faq">FAQ</a>
    <a href="/giris">Sign In</a>
  </div>
</footer>
...
function toggleFaq(el){
  ...
  document.querySelectorAll('.ln-faq-q').forEach(function(q){q.classList.remove('open');});
  document.querySelectorAll('.ln-faq-a').forEach(function(a){a.classList.remove('open');});
  if(!wasOpen){
...
@app.route("/")
def index():
    # ── Landing page: giriş yapmamışsa göster
    if not session.get("user") or session.get("rol") in ("misafir",):
        return landing_page()

    if session.get("rol") in ("goruntuleyici",):
